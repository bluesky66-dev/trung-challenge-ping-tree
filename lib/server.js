...
const { targetHandler, oneTargetHandler, decideRoute } = require('./controllers/target.controller')
...
router.set('/api/target/:id', oneTargetHandler)
router.set('/api/targets', targetHandler)
router.set('/route', decideRoute)
...
function onError (req, res, err) {
  if (!err) return
  res.statusCode = err.statusCode ? err.statusCode : res.statusCode !== 200 ? res.statusCode : 500
